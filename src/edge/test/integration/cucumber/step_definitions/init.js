...
var environment = config['demo-helloWorld-v1'].env;
...
console.log('&$demo-helloWorld-v1 api: [' +  domains[environment] + ', ' + config['demo-helloWorld-v1'].basepath + ']');

BeforeAll("BeforeFeatures", function(next) {
    if (apps[0].name) {
        console.log ("Getting Creds from DevAppsKeys.json");
        getCreds(config['demo-helloWorld-v1'].app, config['demo-helloWorld-v1'].product);
    } else
    {
...
Before(function(scenario, callback) {
    ...
    this.apickli = new apickli.Apickli(tokens[0],
        tokens[1]+ config['demo-helloWorld-v1'].basepath,
        './test/integration/features/fixtures/');
    callback();
});
